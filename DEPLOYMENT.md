...
These genuinely are opt-in — the app works without them, and each feature just silently no-ops (or shows a "check Supabase setup" error) until its migration is run. `baseline.sql` includes `0002`–`0006`, `0008`, `0010`, `0011`, and `0012` — the table below is for applying them individually to an **existing** project that hasn't run them yet (run `0001` first, then any of these you want, in any order):
