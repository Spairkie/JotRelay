...
2. `_parseRoute()` (`src/app/routing.js`) strips the configured base path (`window.SYNCPAD_CONFIG.basePath`, `/JotRelay`) from `location.pathname` and classifies the route — a room id, `/admin`, `/contact`/`/privacy`/`/terms`, `/share/:token`, or the landing screen.
...
3b. If the room has a future `reveal_at` and the visiting device isn't `created_by_device`, `joinRoom()` shows `#reveal-screen` (a countdown, no submit) instead of proceeding — checked before the passcode/encryption gates below, since there's no reason to prompt for either if the visitor can't see the content yet regardless. A one-shot timer re-runs `joinRoom()` once `reveal_at` passes, so the wait auto-resolves without a manual reload.
...
Navigations are split by whether the path looks like a real file (has an extension, e.g. `presskit/video/demo.mp4` opened in a new tab) or an in-app route (no extension, e.g. `/JotRelay/<room-id>`, `/JotRelay/admin`): only route-shaped navigations are mapped to the cached `index.html` shell. A direct navigation to an actual file always serves that file's own content, never the app shell, regardless of which worker is in control.
