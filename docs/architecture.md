...
`service-worker.js` implements **cache-first** for every same-origin GET request, scoped to the same open `CACHE_VERSION` cache. The cached copy is returned immediately if present; a cache miss falls back to the network (and, for navigations specifically, to the offline error response if that also fails).
...
Each cache slot is treated as **immutable once populated**, with one narrow exception: a genuine cache **miss** is still written back on that request (self-healing a slot the install handler's per-asset `.catch()` tolerance left empty after a transient failure — otherwise offline support for that one asset would stay broken until the next deploy, even though every online visit succeeds). An already-*hit* slot is never overwritten at request time — an earlier version of this handler did overwrite on every request opportunistically, which reads as harmless but isn't: a currently-active worker keeps answering every fetch for pages it already controls for as long as it takes a newer worker to actually take over, and the network always serves whatever is *currently deployed* regardless of which worker generation is asking. Repeated per-request overwrites during that window could drift the "coherent" `CACHE_VERSION` cache entry-by-entry as the user browsed — e.g. picking up a newly-deployed `app.js` while every module it imports is still the old cached copy. Write-once-on-miss can't reproduce that: once a slot is filled it hits on every later request for the rest of that generation's lifetime and is never written to again — so the only way a *populated* slot's contents change is a whole new `CACHE_VERSION` being precached atomically on the next deploy.
