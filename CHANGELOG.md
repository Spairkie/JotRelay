...
- **Five real bugs found in `0010`/`0011` during review of this same change, all fixed and re-verified against a real Postgres instance rather than assumed correct from the diff:**
  ...
  - `0011`'s `file_no` assignment could reissue a deleted file's number to a new upload (`max(file_no)+1` recomputed from *currently existing* rows), silently redirecting any existing `syncpad-file:N` note reference to the wrong file. Replaced with a persistent, monotonic per-room counter (`syncpad_rooms.file_no_seq`) that never decreases regardless of deletions; verified a deleted number is never reissued.
