...
- **Twelve real bugs found in `0010`/`0011` across four rounds of review of this same change, all fixed and re-verified against a real Postgres instance rather than assumed correct from the diff:**
  ...
  - The upgrade-seed buffer's `max(file_no) + 50000000` was plain `integer` arithmetic, and — via the previous bug — a caller could have already set some room's `file_no` close to the int4 max, which would overflow that expression and abort the *entire* migration for *every* room, not just the affected one. Fixed by computing the seed in `bigint` and clamping it to the int4 max before storing it back into the `integer` column; verified with a simulated pre-existing corrupted row that the migration completes instead of erroring (a room in that state can still eventually exhaust its own headroom on a later upload, a narrow, self-contained consequence of a since-closed exploit rather than a migration-wide failure).
