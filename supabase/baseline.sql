...
-- 0011, 0012, 0013, 0014, 0015, in that order. Every statement in every section
-- below is idempotent (CREATE IF NOT EXISTS / CREATE OR REPLACE / DROP
-- POLICY IF EXISTS + CREATE POLICY), so this whole file is safe to paste
-- into the Supabase SQL Editor and run in ONE shot for a brand-new
-- project — no need to run 12 separate files in order, and no historical
-- migration-numbering questions to work through.
--
-- Verified: the 0001-0011 portion was run twice in a row against a real
-- Postgres 16 server (stubbed with minimal auth/storage schemas standing
-- in for the Supabase platform pieces this SQL assumes exist) — zero
-- errors either time, confirming both that the concatenation is
-- syntactically sound and that the whole file is genuinely idempotent, not
-- just each individual section in isolation. 0012-0014 were appended
-- afterward (each individually small — an index, a column, a constraint —
-- and already idempotent in their own source files) without repeating that
-- same from-scratch two-pass test; 0013 has separately been applied and
-- confirmed working against this project's actual live database (see
-- comments.js's PGRST204 fix and CHANGELOG.md), but the concatenated
-- 0001-0014 file as a single unit has not been re-run end-to-end against a
-- fresh instance.
--
-- Excluded on purpose:
...
-- ============================================================
-- SOURCE: supabase/migrations/0015_timed_reveal.sql
-- ============================================================
-- Timed-reveal room setting — see that migration's own header for the full
-- rationale (client-side time gate, mirror image of expires_at, no new
-- table or RPC needed).
