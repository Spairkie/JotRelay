...
- **Timed reveal (`reveal_at`) is a join-time-only, client-only gate — same caveat as passcode, plus its own scoping quirk.** `joinRoom()` (`src/app/room-lifecycle.js`) shows `#reveal-screen` instead of the room whenever `room.reveal_at` is in the future and the visiting device isn't `room.created_by_device` (same creator-exemption pattern View-once and Device limit already use). Two things to keep in mind before touching this: (1) it's a pure `SELECT`-time comparison with no Postgres enforcement, so a technical visitor querying the REST API directly still sees the content early — don't describe it as confidential. (2) It only gates at *join* time — a non-creator device already inside a room (i.e., it joined before `reveal_at` was set) is **not** retroactively kicked out or blanked when the creator turns timed-reveal on remotely, unlike encryption's `_enterEncryptedNoKeyMode()`, which actively clears already-rendered content because that content really does become undecryptable. Timed reveal has no equivalent cryptographic reason to do that, so it deliberately doesn't try.
